...
class NativeAgentCommandLineBuilder : CommandLineBuilder {
    override fun build(
        ...
        return listOf(
            "-agentpath:$nativeAgentFile="
                    + "drillInstallationDir=${agentDir.absolutePath},"
                    + getArgsMap(configuration).map { (k, v) -> "$k=$v" }.joinToString(",")
        )
    }

    private fun findNativeAgentFile(agentDir: Directory): String {
        return findFile(agentDir, currentOsLibExt)?.absolutePath
